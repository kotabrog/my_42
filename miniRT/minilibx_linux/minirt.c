...
/*   Updated: 2020/12/20 14:22:42 by ksuzuki          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		choice_object(t_data *img, int point[2])
{
	...
	direction = vec_add(&(img->screen), &temp);
	vec_set_inplace(&temp, 0, 0, 0);
	while (i < img->n_object)
	{
		d = calc_discriminant(&temp, &direction, &(img->objects[i]));
		if (is_true_discriminant(&(img->objects[i]), d))
		{
			d = calc_solution(&temp, &direction, &(img->objects[i]), d);
...
void	calc_cos(t_data *img, t_vec *l_point, double cos_arr[2])
{
	...
	normal_v = calc_normal(&(ob_point), &(img->objects[img->front_ob_i]), img);
