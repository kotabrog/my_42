...
/*   Updated: 2020/12/26 01:32:06 by ksuzuki          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line_error(char *memo, char **line)
{
	free(*line);
	free(memo);
	*line = NULL;
...
int		get_next_line(int fd, char **line)
{
	...
	find_flag = 0;
	if (fd < 0 || line == NULL)
		return (get_next_line_error(memo, line));
	rest = ft_strchr_ex(memo, '\n', &n, &find_flag);
	if (rest && (rest = ft_strndup(rest, -1)) == NULL)
		n = -1;
	if (n >= 0 && (*line = ft_strndup(memo, n)) == NULL)
		n = -1;
	if (n >= 0 && !find_flag)
