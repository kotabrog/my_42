...
/*   Updated: 2020/12/26 01:46:53 by ksuzuki          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		copy_data(t_data *dist, t_data *src)
{
	*dist = *src;
	...
	if (!(dist->objects = (t_object *)malloc(sizeof(t_object) * \
												dist->n_object)))
		return (ERROR_MALLOC);
...
void	set_img(t_vars *vars, t_data *img, t_data **img_array, void **mlx)
{
	...
	if (!(*img_array = (t_data *)malloc(sizeof(t_data) * img->n_camera)))
		exit_error(vars, ERROR_MALLOC, POINT_WIN_FREE);
	while (i < img->n_camera)
	{
		...
		if (((*img_array)[i].img = mlx_new_image(*mlx, (*img_array)[i].x,\
								(*img_array)[i].y)) == NULL)
		{
			img->n_camera = i + 1;
			exit_error(vars, ERROR_MLX, POINT_DATAS_FREE);
		}
		(*img_array)[i].now_camera = i;
		(*img_array)[i].addr = mlx_get_data_addr((*img_array)[i].img, \
				&((*img_array)[i].bits_per_pixel), \
				&((*img_array)[i].line_length), &((*img_array)[i].endian));
