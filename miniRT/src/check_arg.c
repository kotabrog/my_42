...
/*   Updated: 2020/12/26 01:53:29 by ksuzuki          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_line_info(t_vars *vars, int *flag, int fd, t_type *type_flag)
{
	char	*buff;
...
void	get_object_data(t_vars *vars, char *argv)
{
	...
	get_line_info(vars, &flag, fd, &type_flag);
	close(fd);
	if (flag)
		exit_error(vars, flag, POINT_DATA_FREE);
	if (!type_flag.resolution || vars->img.n_camera == 0)
		exit_error(vars, ERROR_MISSING, POINT_DATA_FREE);
	if (vars->img.n_object > MAX_OB || vars->img.n_light > MAX_OB || \
			vars->img.n_camera > MAX_OB)
		exit_error(vars, ERROR_OVER_OB, POINT_DATA_FREE);
}
